...
入口：游戏右上角「📅 代煎排期」按钮，打开排期面板（`src/scheduler-ui.ts`，纯 DOM 实现，与画布游戏互不干扰）。

- **倒排**：录入「开始喝药日、一剂喝几天、一共几剂」，按 `开始喝药日 → 取药日 → 送煎日 → 抓药日` 逐剂往回倒排（提前量可在设置里调）。
- **歇业日前挪**：每周固定歇业日 + 指定节假日，落在歇业日的抓药/送煎/取药日一律往前挪，并在「调整说明」列写明挪动原因。
- **煎炉容量**：每日煎炉容量（剂）可配；当天排不下的剂次往后顺延（跳过歇业日），取药日跟着顺延；取药日晚于开始喝药日时给出「断药」预警。若取药日被手动固定，则送煎往前提以保住取药日。
- **手动改期**：表格里直接改任意一剂的日子，该剂后续所有日子按链式规则重算；手动指定的日子撞歇业/炉满时给提醒，可一键撤销手动改动。
- **拥挤提醒**：同一取药日的病人数达到提醒线时，面板顶部红色预警并列出病人姓名。
- 数据存 localStorage（`decoct-scheduler-v1`），只存录入与手动改动，重开页面按当前参数重算。
- 排期逻辑全部在 `src/scheduler.ts` 纯函数中（`computePlan` / `withDateOverride` / `pickupCongestion` 等），单测见 `tests/scheduler.test.ts`。
